...
def _request_json(url: str, params: dict[str, Any], max_attempts: int = 5) -> dict[str, Any]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if response.status_code == 429 and attempt < max_attempts:
            log(f"Rate limited by weather forecast API, retrying attempt {attempt + 1}")
    ...
    raise RuntimeError("Unable to retrieve weather forecast after retries")
...
def _rows_from_payload(
    ...
    for field in WEATHER_FIELDS:
        ...
        if len(values) not in {0, len(times)}:
            raise ValueError(f"Hourly forecast field {field} length {len(values)} does not match time length {len(times)}")
...
def read_weather_forecast(context: ComponentExecutionContext) -> pd.DataFrame:
    ...
    log(
        f"Returning {len(forecast)} forecast weather rows for {FORECAST_DAYS} days "
        f"generated at {forecast_generated_at.isoformat()}"
    )
    return forecast